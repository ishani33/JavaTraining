...
class RecursiveSum extends RecursiveTask<Long> {
	...
	protected Long compute() {
		...
		if (size < THRESHOLD) {
			...
			return right.compute() + left.join(); //we explicitly call compute on right to use the thread which is free and not create a new thread
		// we could have used right.fork() also here but it would unnecessarily create threads when one of the thread is idle in the thread pool
		}
	}
}
